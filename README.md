...
*   **Luồng Upload Ảnh (`POST /api/images`):**
    ...
    6.  **Broadcast (Real-time):** Gửi metadata ảnh mới tới tất cả client đang kết nối qua SignalR Hub (`ReceiveNewImages`).
...
*   **Quản lý State và API (`frontend/src/app/services/image.ts`):**
    ...
    *   **SignalR Integration:** Khởi tạo kết nối tới Server Hub và lắng nghe sự kiện để cập nhật dữ liệu thời gian thực.
    *   Sử dụng **RxJS Subject** (`imageUploadedSubject`) để phát tín hiệu cập nhật toàn ứng dụng khi có dữ liệu mới từ API hoặc SignalR.
...
### 3. Đồng bộ Real-time (SignalR)
Ứng dụng tích hợp **ASP.NET Core SignalR** để thiết lập kênh giao tiếp hai chiều (Full-duplex) giữa Server và Client. Điều này cho phép server chủ động đẩy dữ liệu (Push) tới trình duyệt ngay khi có ảnh mới, thay vì trình duyệt phải liên tục hỏi server (Polling).

*   **Đặc điểm kỹ thuật:**
    *   **Abstraction Transport:** SignalR tự động chọn giao thức truyền tải tốt nhất dựa trên khả năng của trình duyệt: WebSockets (ưu tiên), Server-Sent Events, hoặc Long Polling.
    *   **Khả năng tự phục hồi:** Frontend được cấu hình với `.withAutomaticReconnect()` để duy trì kết nối ổn định ngay cả khi mạng chập chờn.

*   **Triển khai tại Backend (`backend/Program.cs`):**
    *   **Hub Definition:** `ImageHub` đóng vai trò là trung tâm điều phối tin nhắn.
    *   **Broadcast Mechanism:** Khi API `POST /api/images` hoàn tất xử lý metadata, nó sử dụng `IHubContext<ImageHub>` để gọi phương thức `ReceiveNewImages`. 
    *   **Dữ liệu truyền tải:** Một mảng các Object chứa thông tin ảnh (ID, Name, URL, Tags) được gửi đồng loạt tới tất cả Client đang active.

*   **Triển khai tại Frontend (`frontend/src/app/services/image.ts`):**
    *   **Connection Lifecycle:** Kết nối được khởi tạo ngay khi ứng dụng Angular load.
    *   **Listener Logic:** Khi nhận sự kiện `ReceiveNewImages`, `ImageService` đẩy dữ liệu vào một **RxJS Subject**.
    *   **Reactive UI:** Component `Gallery` đăng ký (subscribe) vào Subject này. Khi có dữ liệu mới, danh sách ảnh hiển thị sẽ được cập nhật tức thì (Reactive Update) mà không cần reload trang.
