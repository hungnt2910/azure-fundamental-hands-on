...
Để mang lại trải nghiệm mượt mà, ứng dụng sử dụng **ASP.NET Core SignalR** để đồng bộ hóa danh sách ảnh tức thì:
*   **Hub Configuration:** Một `ImageHub` được thiết lập tại endpoint `/hubs/images`.
*   **Cơ chế Broadcast:** Ngay sau khi ảnh được xử lý xong ở Backend, server gọi `Clients.All.SendAsync("ReceiveNewImages", ...)` để đẩy dữ liệu tới tất cả trình duyệt đang mở ứng dụng.
*   **Xử lý phía Client:** Frontend sử dụng `@microsoft/signalr` để duy trì kết nối persistent. Khi nhận được thông báo, Gallery sẽ tự động chèn ảnh mới vào đầu danh sách với hiệu ứng mượt mà mà không cần tải lại trang.
